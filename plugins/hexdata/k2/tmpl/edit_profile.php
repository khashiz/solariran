...
window['loadfields'] = function()
{
	...
	$hd.ajax({
		  url: "index.php",
		  type: "POST",
		  data: {'option':'com_hexdata', 'view':'profiles', 'task':'plugintaskajax', 'plugin':'k2.load_fields', 'type':$hd('select#paramstype').val(), 'profileid':<?php echo (int)$profile->id; ?>, "<?php echo JSession::getFormToken(); ?>":1, 'abase':1},
		  beforeSend: function()	{
			$hd(".loading").show();
		  },
		  complete: function()	{
...
<legend class="k2_options"><?php echo JText::_( 'ATTRIBS' ); ?></legend>
...
<input type="hidden" name="params[table]" id="paramstable" value="#__k2_items" />